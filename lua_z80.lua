...
local function ADD_to_IX_string(hi, reg32)
    -- basically a simple add followed by an ADC H,x.
    -- maybe we should try a 16 bit add? (but then we have to split anyway...)
    return string.format(
[[  
    ...
    CPU._F = CPU.Carry + CPU:get_F_only_SZV() + bit32.band(bit32.bxor(bit32.rshift(result, 8), %s, bit32.rshift(CPU.IX, 8)),Z80_H_FLAG)
    CPU.IX = result ]], reg32, hi)
end
decode_DD_instructions[0x09] = ADD_to_IX_string("CPU.B", "CPU.B*256+CPU.C")
...
-- inc IXL
decode_DD_instructions[0x2C] = "temp = (CPU.IX+1)%256 CPU.IX=bit32.band(CPU.IX, 0xFF00)+temp " .. 
            string.format(inc_flag_calc, "temp", "temp", "temp")

-- inc IYH
decode_FD_instructions[0x24] = "CPU.IY=(CPU.IY+256)%65536 temp=bit32.band(CPU.IY, 0xFF00)/256 " .. 
            string.format(inc_flag_calc, "temp", "temp", "temp")

-- inc IYL
decode_FD_instructions[0x2C] = "temp = (CPU.IY+1)%256 CPU.IY=bit32.band(CPU.IY, 0xFF00)+temp " .. 
            string.format(inc_flag_calc, "temp", "temp", "temp")

-- dec IXH
decode_DD_instructions[0x25] = "CPU.IX=(CPU.IX-256)%65536 temp=bit32.band(CPU.IX, 0xFF00)/256 " .. 
